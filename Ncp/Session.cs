...
namespace Ncp
{
	public class Session
	{
		...
		public uint GetResendSequenceAsync()
		{
			...
			var value = tasks.FirstValueAsync(cts);
			if (value == null)
			{
		...
		public uint? GetSendSequenceAsync()
		{
			var cts = new CancellationTokenSource();
			var tasks = new List<Task<uint?>>
			{
		...
		public void ReceiveWithClientAsync(string localClientId, string remoteClientId, byte[] buffer)
		{
			if (this.IsClosed)
			{
		...
		public void DialAsync(int dialTimeout)
		{
			if (this.isAccepted)
			{
			...
			this.SendHandshakePacketAsync(dialTimeout);
		...
		public void AcceptAsync()
		{
			...
			this.SendHandshakePacketAsync(this.Options.MaxRetransmissionTimeout);
		}

		public byte[] ReadAsync(int maxSize = 0)
		{
			try
			{
				if (this.IsClosed)
		...
		public void WriteAsync(byte[] data)
		{
			try
			{
				...
				if (this.IsStream)
				{
					while (data.Length > 0)
					{
						var sendWindowAvailable = this.WaitForSendWindowAsync(this.writeContext, 1);
				...
				{
					this.WaitForSendWindowAsync(this.writeContext, data.Length);
		...
		{
			this.readContext.CancelAsync();
			this.writeContext.CancelAsync();

			var timeout = Channel.CreateBounded<uint?>(1);

			if (this.Options.Linger > 0)
			{
				Thread.Sleep(this.Options.Linger);
				timeout.CompleteAsync();
			}
		...
		private void StartCheckBytesReadAsync()
		{
			while (true)
			{
		...
		private uint WaitForSendWindowAsync(Context context, int n)
		{
		...
		private void FlushSendBufferAsync()
		{
			uint sequenceId = 0;
		...
		private void SendHandshakePacketAsync(int timeout)
		{
			var packet = new Packet
			{
				IsHandshake = true,
		...
		private void SendClosePacketAsync()
		{
			if (this.isEstablished == false)
			{
				throw new SessionNotEstablishedException();
