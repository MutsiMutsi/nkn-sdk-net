	...
	public class Client : ITransactionSender
	{
			...
			SendOptions options = null)
		{
			...
			try
			{
				...
				return HandleSendResultTaskAsync(resultTask, options, responseChannel, timeoutChannel);
			}
			catch (Exception)
			{
			...
			PublishOptions options = null)
		{
			...
			return this.SendAsync<byte[]>(subscribers.ToList(), text, SendOptions.NewFrom(options));
		}
			...
			PublishOptions options = null)
		{
			...
			return this.SendAsync<byte[]>(subscribers.ToList(), data, SendOptions.NewFrom(options));
		}
		...
		public GetSubscribersWithMetadataResult GetSubscribersWithMetadataAsync(
			string topic,
			PublishOptions options = null)
		{
			options = options ?? new PublishOptions();
			...
			if (string.IsNullOrWhiteSpace(this.Wallet.Options.RpcServerAddress))
			{
				try
				{
					...
					return NknSdk.Wallet.Wallet.GetSubscribersWithMetadataAsync(topic, mergedOptions);
				}
				catch (Exception)
				{
				}
			...
			return NknSdk.Wallet.Wallet.GetSubscribersWithMetadataAsync(topic, walletOptions);
		}
		...
		public Amount GetBalanceAsync(string address = "", WalletOptions options = null)
		{
			...
			if (string.IsNullOrWhiteSpace(this.Wallet.Options.RpcServerAddress))
			{
				try
				{
					...
					var balanceResult = NknSdk.Wallet.Wallet.GetBalanceAsync(addr, options);
			...
			var result = NknSdk.Wallet.Wallet.GetBalanceAsync(addr, this.Wallet.Options);
		...
		internal byte[] SendDataManyAsync<T>(
			IEnumerable<string> destinations,
			byte[] data,
			SendOptions options,
		...
		internal byte[] SendPayloadAsync(
			string destination,
			Payload payload,
			bool isEncrypted = true,
			uint maxHoldingSeconds = 0)
		{
			var processedDestination = this.ProcessDestinationAsync(destination);
		...
		internal byte[] SendPayloadManyAsync(
			IEnumerable<string> destinations,
			Payload payload,
			bool isEncrypted = true,
			uint maxHoldingSeconds = 0)
		{
			...
			{
				return this.SendPayloadAsync(destinations.First(), payload, isEncrypted, maxHoldingSeconds);
			}

			var processedDestinations = this.ProcessDestinationManyAsync(destinations);
		...
		internal IList<string> ProcessDestinationManyAsync(IEnumerable<string> destinations)
		{
			if (destinations.Count() == 0)
			{
			...
			return destinations.Select(this.ProcessDestinationAsync).ToArray();
		...
		internal string ProcessDestinationAsync(string destination)
		{
			...
			{
				var registrantResponse = this.GetRegistrantAsync(destinationParts[destinationParts.Length - 1]);
		...
		private void ConnectAsync()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				try
				{
					if (useTls)
					{
						getWsAddressResult = RpcClient.GetWssAddress(this.options.RpcServerAddress, this.Address);
					}
					else
					{
						getWsAddressResult = RpcClient.GetWsAddress(this.options.RpcServerAddress, this.Address);
					}
				}
				catch (Exception ex)
				{
